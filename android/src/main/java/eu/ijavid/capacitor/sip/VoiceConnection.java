...
import static eu.ijavid.capacitor.sip.Constants.ACTION_ANSWER_CALL;
import static eu.ijavid.capacitor.sip.Constants.ACTION_AUDIO_SESSION;
import static eu.ijavid.capacitor.sip.Constants.ACTION_DID_CHANGE_AUDIO_ROUTE;
import static eu.ijavid.capacitor.sip.Constants.ACTION_DTMF_TONE;
import static eu.ijavid.capacitor.sip.Constants.ACTION_END_CALL;
import static eu.ijavid.capacitor.sip.Constants.ACTION_HOLD_CALL;
import static eu.ijavid.capacitor.sip.Constants.ACTION_MUTE_CALL;
import static eu.ijavid.capacitor.sip.Constants.ACTION_ON_SILENCE_INCOMING_CALL;
import static eu.ijavid.capacitor.sip.Constants.ACTION_SHOW_INCOMING_CALL_UI;
import static eu.ijavid.capacitor.sip.Constants.ACTION_UNHOLD_CALL;
import static eu.ijavid.capacitor.sip.Constants.ACTION_UNMUTE_CALL;
import static eu.ijavid.capacitor.sip.Constants.EXTRA_CALLER_NAME;
import static eu.ijavid.capacitor.sip.Constants.EXTRA_CALL_NUMBER;
import static eu.ijavid.capacitor.sip.Constants.EXTRA_CALL_UUID;
...
public class VoiceConnection extends Connection {
    ...
    public void reportDisconnect(int reason) {
        super.onDisconnect();
        switch (reason) {
            case 1 -> setDisconnected(new DisconnectCause(DisconnectCause.ERROR));
            case 2, 5 -> setDisconnected(new DisconnectCause(DisconnectCause.REMOTE));
            case 3 -> setDisconnected(new DisconnectCause(DisconnectCause.BUSY));
            case 4 -> setDisconnected(new DisconnectCause(DisconnectCause.ANSWERED_ELSEWHERE));
            case 6 -> setDisconnected(new DisconnectCause(DisconnectCause.MISSED));
            default -> {
            }
        }
        ((SipPhoneConnectionService) context).deinitConnection(handle.get(EXTRA_CALL_UUID));
     ...
     */
    private void sendCallRequestToActivity(final String action, @Nullable final HashMap<String, String> attributeMap) {
